...
public class DatePickerDialog extends AlertDialog implements
	...
	private LinearLayout lin_dataend;
	...
	public DatePickerDialog(Context context, int theme,
			OnDateSetListener callBack, int year, int monthOfYear,
			int dayOfMonth,  boolean isBegin) {
		super(context, theme);
		...
		setView(view);
	TextView	tv_data_title = (TextView)view.findViewById(R.id.tv_data_title);
		...
		// 如果要隐藏当前日期，则使用下面方法。
		if (!isBegin) {
			tv_data_title.setText("结束时间");
	}else{
		tv_data_title.setText("开始时间");
	}
	}
	/**
	 * 隐藏DatePicker中的日期显示
