...
public class SetFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (view == null) {
			...
			tv_modifyip.setVisibility(View.VISIBLE);
	...
	private void clearCache() {
		try {
			System.out.println(new File(SDCardTools.getFilePath().toString()));
	...
	private void onClickListener() {
		...
		loginLayout.setOnClickListener(new OnClickListener() {// 跳转到登录
					...
					public void onClick(View arg0) {
						...
						BUserlogin.set_intent_login = true;
		...
		exitBtn.setOnClickListener(new OnClickListener() {// 跳转到登录
			...
			public void onClick(View arg0) {
				...
				if (BUserlogin.loginStatus == 1) {
					...
					BUserlogin.Username = "";
					login_head_img
							.setImageResource(R.drawable.detail_icon_user_default);
					changeLoginState();
		...
		tv_modifyip.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (android.os.Build.VERSION.SDK_INT > 11) {
					...
					builder = new AlertDialog.Builder(getActivity()); // 先得到构造器
				}

				builder.setTitle("当前IP:" + AppConfig.getIp(getActivity()))
						// 提示框标题
						.setView(view)
						.setPositiveButton(
								"修改",// 提示框的两个按钮
	...
	Handler mainHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 101:
				...
				break;
			case 201:// 清除缓存
				txt_cachesize.setText((double) size + "M");
	...
	private void setCacheSize() {
		try {
			// 文件大小
			size = (double) (GetFileCacheSize.getFolderSize(new File(
					SDCardTools.getFilePath().toString())));
