...
public class ASearchSchedule {
	...
	public int analySearchSchedule(String result,
			List<HashMap<String, String>> list, boolean isClear, int imp) {
		// imp 接口类型
		int type = 1;
		try {
				...
				for (int i = 0; i < length; i++) {
					...
					for (int j = 0; j < jsonArr_lenth; j++) {
						...
						if (BIntentObj.isGetClass == 1) {
							String Teacher = ((JSONObject) jsonArr.get(j))
									.getString("Teacher");
							hashmap.put("Teacher", Teacher);// 类型
						}
						...
						hashmap.put("Id",
								((JSONObject) jsonArr.get(j)).getString("Id"));// 类型
						list.add(hashmap);
