		...
		try {
			...
			//	System.out.println(mNextoBbj.toString());
				...
				if(mBTaskInfo.isend){
					...
					if(mNextoBbj!=null&&!mNextoBbj.equals("")){
					int i = nextnodes.length();
					JSONObject obj = null ;
					for (int j = 0; j < i; j++) {
						 obj = (JSONObject) nextnodes.get(j);
						...
						mBMeetingApplyLeade.setRecname(obj.getString("recname"));
						mBMeetingApplyLeade.setReceNo(obj.getString("rec"));
