...
public class UnEducationListActivity extends BaseActivity {
		...
		public void handleMessage(Message msg) {
			...
			case -1:
				...
				if (listAll.size() < 1) {
					...
					textView_show.setText("暂时没有您的非教学工作安排!");
				}else{
			...
			case 0:
				// 获取列表数据后
				Toast.makeText(getApplicationContext(), "您还未登陆，请登录后操作", 0)
						.show();
				mProgram_lv.onRefreshComplete();
				if (listAll.size() < 1) {
					rel_show.setVisibility(View.VISIBLE);
	...
	protected void initView() {
		...
		unedu_adapter = new UnEducationListAdapter(
				UnEducationListActivity.this, listAll);
	...
	protected void setListener() {
		...
		mProgram_lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> adapter, View view,
				...
				Intent mIntent = new Intent(UnEducationListActivity.this,
						UnEduProgramDetailActivity.class);
				mIntent.putExtra("workId", listAll.get(position - 1)
						.getWorkId());
	...
	protected void setView() {
		...
		mProgram_lv.getRefreshableView().setSelector(android.R.color.transparent);
					...
					new RequestCallBack<String>() {
						...
						public void onSuccess(ResponseInfo<String> responseInfo) {
							...
							try {
								...
								Log.e("getDateByHttp>>>==", "listAll.size=="
										+ listAll.size());
