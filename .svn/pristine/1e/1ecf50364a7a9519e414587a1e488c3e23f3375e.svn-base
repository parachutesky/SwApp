...
public class MeetingApply extends BaseActivity {
	// 返回的数据解析
	private Button btn_meetingapply_check, btn_positiveButton,
			btn_negativeButton;
	...
	private TextView tv_meetingcheck_title_name, tv_flow_name,
			tv_state_overshenpi, tv_meetingcheck_taskname;// tv_state_overshenpi
															// 头部 待审批的 显示控件
	...
	private final String FROM_VALUE1 = "HadDidFragment";
	private boolean isendd;
	private boolean isreturn;// 是不是可以归档且驳回
	private boolean ispush = false;// 是不是推送过来的
	...
	protected void initView() {
		...
		btn_positiveButton = (Button) findViewById(R.id.btn_positiveButton);
		btn_negativeButton = (Button) findViewById(R.id.btn_negativeButton);
		...
		tv_meetingcheck_taskname = (TextView) findViewById(R.id.tv_meetingcheck_taskname);//
	...
	private void setInVistibility() {
		...
		btn_positiveButton.setVisibility(View.GONE);
	...
	protected void onResume() {
		...
		super.onResume();
		if (BIntentObj.END_MEETINGAPPLY) {
			BIntentObj.END_MEETINGAPPLY = false;
	...
	private void setVistibility() {// 判断是否有数据
		...
		if (!isFrom()) {
			...
			if (isendd) {
				btn_meetingapply_check.setVisibility(View.VISIBLE); // 当 从 待办
																	// 查看时 显示
				if (isreturn) {
					btn_positiveButton.setVisibility(View.GONE);
				...
				btn_meetingapply_check.setVisibility(View.GONE);// 当 从 已办 查看时
																// 隐藏掉
				// 审批button
				btn_positiveButton.setVisibility(View.VISIBLE);
				btn_negativeButton.setVisibility(View.VISIBLE);
			}
		}
	...
	protected void setListener() {
		...
		btn_positiveButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				// 赋值
				intent.putExtra("isagree", 1);// 1表示同意 2表示不同意
				intent.setClass(MeetingApply.this, TaskAuditingActivity.class);
				startActivity(intent);
			}
		});
		btn_negativeButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				Intent intent = new Intent();
				// 赋值
				intent.putExtra("isagree", 2);// 1表示同意 2表示不同意
				intent.setClass(MeetingApply.this, TaskAuditingActivity.class);
				startActivity(intent);
			}
		});
	...
	public void flowEnd() {
		...
		String url = AppConfig.getIp(MeetingApply.this)
				+ AppConfig.METHOAD_FLOWEND_OA;
		LogUtils.e("url:" + url);
		http.send(HttpRequest.HttpMethod.POST, url, params,
				new RequestCallBack<String>() {
					...
					public void onFailure(HttpException arg0, String arg1) {
						// TODO Auto-generated method stub
						ToastViewTools.initToast(MeetingApply.this, "网络超时....");
					}
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						...
						try {
							...
							if (null != status && status.equals("200")) {
								BIntentObj.IsDoProcess = true; // 归档成功之后 将
																// 获取流程设置为false
																// 为了刷新代办页面
								BIntentObj.IsEndProcess = true;
	...
	final PopClickListening popClickListenershow = new PopClickListening() {
		...
		public void setListViewForName(ListView lv, LinearLayout lin) {
			...
			try {
				 ...
				 * else {
				 * 
				 * if (k == 1) { uPDate(0); pop_leaderid =
				 * mBTaskInfo.list_getnode.get(0) .getNodeid(); // 获取下一节点的 // id
				 * recepterName = mBTaskInfo.list_getnode.get(0) .getReceNo();
				 * // 获取接收人的账号
				 * 
	 ...
	 */
	/*
	 * public void uPDate(int arg2) { int kj = mBTaskInfo.list_getnode.size();
	 * for (int i = 0; i < kj; i++) { if (i == arg2) {
	 * mBTaskInfo.list_getnode.get(i).setIdididi(arg2 + ""); } else {
	 ...
	 * 得到任务详情
	 */
	private void getWorkInfo_All(String loginName, String workID,
			String flowID, String fid) {
		RequestParams params = new RequestParams(); // 参数
		params.addBodyParameter("loginName", loginName);
		params.addBodyParameter("workID", workID);
		params.addBodyParameter("flowID", flowID);
		params.addBodyParameter("IsDaiban", isFrom() + "");
		...
		String url = AppConfig.getIp(MeetingApply.this)
				+ AppConfig.METHOAD_GETTASKINFO_OA;
				...
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						...
						mBTaskInfo = mATaskInfo.analysisTaskInfo(
								...
								BIntentObj.IsAna);
						isendd = mBTaskInfo.isend;
						isreturn = BIntentObj.isreturn;
	...
	private class MyTask extends AsyncTask<String, Integer, String> {
		...
		protected void onPreExecute() {
			...
			tv_meetingcheck_taskname.setText(BIntentObj.mBUserTasks
					.getTaskName());
			// 开始绘图
			initContentView();
			initlinDetailView();
	...
	private void initlinDetailView() {
		...
		for (int i = 0; i < lenth; i++) {
			...
			System.out.println("title_col项：" + title_col);
			...
			for (int ii = 0; ii < title_col; ii++) {// 为了绘制出一个title
				...
				for (int j = 0; j < title_lenth; j++) {
					...
					tv.setText(mBTaskInfo.detail.get(i).columns.get(ii)[j]);
